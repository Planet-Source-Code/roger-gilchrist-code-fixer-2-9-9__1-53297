...
Private Function AndIsNotBitWise(ByVal strTest As String) As Boolean
  ...
  For I = LBound(TmpA) To UBound(TmpA)
    ...
    If InstrAtPositionArray(TmpA(I), ipAny, False, EqualInCode, " > ", " < ", " <> ", " <= ", " >= ") Then
      IsBool = IsBool + 1
     ElseIf ArrayMember(TmpA(I), "True", "False") Then
      IsBool = IsBool + 1
        ...
        If InstrAtPosition(strWork, ":", ipAny, False) Then
          ...
          If IsGotoLabel(LeftWord(strWork), CompName, , arrGoTo) Then
           ...
           Else
            ...
            'If InStr(strWork, SngSpace) = 0 Then
            ...
            If MultiLeft(ArrRoutine(J), True, "With " & strName) Then
             ...
             Else
              If InCode(ArrRoutine(J), TPos) Then
                'v2.9.3 Thanks Ian this is missing fix
...
Public Sub IfAndThenShortCircuit(cMod As CodeModule)
              ...
              If IfAndThenShortCircuitLine(L_CodeLine) Then
                If IfAndThenShortCircuitSafeToApply(L_CodeLine) Then
                  Select Case FixData(DetectShortCircuit).FixLevel
                   Case CommentOnly
                    ArrRoutine(RLine) = Marker(L_CodeLine, SUGGESTION_MSG & "Short Curcuit: Previous line may be able to be optimized by multiple 'If test_Condition Then' lines", MAfter)
                   Case FixAndComment
                    IfAndThenShortCircuit_Apply ArrRoutine, RLine, L_CodeLine, UpDated
...
Private Sub IfAndThenShortCircuit_Apply(tmpB As Variant, _
  ...
  If TypeOfIf(tmpB, LineNo, EndPos) = Simple Then
    L_CodeLine = Safe_Replace(L_CodeLine, " And ", " Then " & vbNewLine & "If ")
    tmpB(LineNo) = Marker(L_CodeLine, WARNING_MSG & "Short Curcuit: 'If <condition1> And <condition2> Then' expanded." & IIf(Xcheck(XVerbose), RGSignature & "'If <condition1> Then" & RGSignature & "If <condition2> Then '" & RGSignature & "Make <condition1> most likely to fail.", vbNullString), MAfter, Update)
    For K = 1 To CountSubString(L_CodeLine, " Then " & vbNewLine & "If ")
      tmpB(EndPos) = Marker(tmpB(EndPos) & vbNewLine & _
       "End If", RGSignature & "Short Circuit inserted this line", MEoL, Update)
    Next K
  End If
...
Private Function ProtectBoolean(strCode As String, _
  ...
  If MultiRight(strCode, True, "= " & strTF) Then
        ...
        If ArrayMember(LeftWord(MyStr), "Function", "Sub", "Enum", "Event", "Property", "Declare", "WithEvents") Then
          ...
          If CheckDefaultAttribute(cMod, WordInString(ExpandForDetection(MyStr), 2)) Then
           ...
           ElseIf InstrAtPositionArray(MyStr, IpLeft, True, "Declare", "Property") Then
            ...
            ScopeTo = SpaceOffSet & NewScope & MyStr & CommentStore & vbNewLine & _
             WARNING_MSG & "Scope Changed to " & NewScope
           ElseIf InstrAtPositionArray(MyStr, IpLeft, True, "Event") Then
...
':)Code Fixer V2.9.6 (9/02/2005 3:01:41 AM) 10 + 1191 = 1201 Lines Thanks Ulli for inspiration and lots of code.
