...
Private Function BlockNoWithDrawCommands(ByVal varTest As Variant, _
  ...
  If Not BlockNoWithDrawCommands Then
    ...
    BlockNoWithDrawCommands = InstrAtPositionArray(varTest, IpRight, False, ".Print")
  End If
  ...
  For I = lngCounter To UBound(arrR)
    Select Case strTest
     ...
     Case "For"
      ...
      End If
     Case "Select Case"
      If InstrAtPositionArray(arrR(I), IpLeft, True, strTest) Then
        lngEmbeded = lngEmbeded + 1
       ElseIf InstrAtPositionArray(arrR(I), IpLeft, True, "End Select") Then
        lngEmbeded = lngEmbeded - 1
      End If
     Case "If"
      ...
      If LeftWord(arrR(I)) = strTest Then
        ...
        End If
      End If
      If InstrAtPositionArray(arrR(I), IpLeft, True, "End If") Then
        lngEmbeded = lngEmbeded - 1
      End If
     Case "While"
      If LeftWord(arrR(I)) = strTest Then
  ...
  For I = lngCounter To LBound(arrR) Step -1
    Select Case strTest
     ...
     Case "Next"
      ...
      End If
     Case "End Select"
      If InstrAtPositionArray(arrR(I), IpLeft, True, "Select Case") Then
        lngEmbeded = lngEmbeded + 1
       ElseIf InstrAtPositionArray(arrR(I), IpLeft, True, strTest) Then
        lngEmbeded = lngEmbeded - 1
      End If
     Case "End If"
      'v 2.1.6 Thanks to Tom Law who showed me the bug here
    ...
    If InstrAtPositionArray(arrR(lngCounter), IpLeft, True, "Do", "For", "Select Case", "If", "While") Then
     ...
     ElseIf InstrAtPositionArray(arrR(lngCounter), IpLeft, True, "Loop", "Next", "End Select", "End If", "Wend") Then
      ...
      If lngStructStart < Sline Then
        ...
        GoTo ReTry
      End If
     ElseIf InstrAtPositionArray(arrR(lngCounter), IpLeft, True, "Case") Then
      ...
      If lngStructStart < Sline Then
        ...
        GoTo ReTry
      End If
     ElseIf InstrAtPositionArray(arrR(lngCounter), IpLeft, True, "ElseIf") Then
...
Private Sub SuggestWith_Apply(arrR As Variant, _
  ...
  If TopOfWith Then
    ...
    'Do While Not HasLineCont(ArrR(TopOfWith - 1)) Or (HasTestBit(ArrR(TopOfWith - 1), TestBit) And HasWithUsage(ArrR(TopOfWith - 1)))
    Do While HasLineCont(arrR(TopOfWith - 1)) Or (HasTestBit(arrR(TopOfWith - 1), TestBit) And Not HasWithUsage(arrR(TopOfWith - 1)))
      ...
      If MultiLeft(arrR(TopOfWith - 1), True, "Set " & TestBit) Then
        Exit Do
      End If
    ...
    'Do While Not HasLineCont(ArrR(BottomOfWith)) Or (HasTestBit(ArrR(BottomOfWith + 1), TestBit) And HasWithUsage(ArrR(BottomOfWith + 1)))
    ...
    If BottomOfWith - TopOfWith > 1 Then
      ...
      If StructuralIntegrity(TopOfWith, BottomOfWith, arrR) Then
        If MultiLeft(arrR(TopOfWith - 1), True, "'APPROVED(Y) " & TestBit) Then
      ...
      If Not BlockNoWithDrawCommands(strCodeOnly(varCode), strTest) Then
        ...
        For J = 0 To 1
          ...
          For I = 1 To Possible
            ...
            varCode = Safe_Replace(varCode, "- " & strTest & arrCon(J), "- " & arrCon(J))
          Next I
          If MultiLeft(varCode, True, strTest & arrCon(J)) Then
...
Private Function SuggestWithStructureGetTestBit(ByVal strCodeLine As String, _
  ...
  If Not LineUnacceptableForWithTextBit(strCodeLine) Then
    ...
    If TPos Then
      'If InCode(strCodeLine, TPos) Then
          ...
          If LenB(TestBit) Then
            If AcceptableWithTarget(L_CodeLine, TestBit, ".", CountOfWithBits) Or AcceptableWithTarget(L_CodeLine, TestBit, "!", CountOfWithBits) Then
             ...
             Else
              ...
              If MultiLeft(L_CodeLine, True, "With " & TestBit) Then
                If InStr(ArrRoutine(J - 1), SUGGESTION_MSG & " The previous ") = 0 Then
                  For I = TopOfWith To TopOfWith + CountOfWithBits + 1
                    ...
                    'b2.4.3 included ReDim in the list
                    If ArrayMember(strOutsider, "With", "Load", "Set", "Unload", "Next", "RaiseEvent", "End", "ReDim") Then
                      GoTo noPrevMsg
                    End If
...
Public Sub WithStructurePurity(cMod As CodeModule)
  ...
  'Copyright 2003 Roger Gilchrist
  'e-mail: roCurLineagilkrist@hotmail.com
  'Test that if a With structure is used there are no unnecessary references to the obCurLineect
  'before reaching the End with
  'ver 1.1.10 commentting added
  ModuleNumber = ModDescMember(cMod.Parent.Name)
  If dofix(ModuleNumber, WithPurity) Then
    ArrMember = GetMembersArray(cMod)
    MemberCount = UBound(ArrMember)
    If MemberCount > 0 Then
      For Member = 1 To MemberCount
        If InStr(ArrMember(Member), "With ") Then
          ArrRoutine = Split(ArrMember(Member), vbNewLine)
          MemberMessage GetProcNameStr(ArrMember(Member)), Member, MemberCount
          ProcWithStructurePurify ArrRoutine, UpDated, MissingWith, Embedded
          If MissingWith Then
            GoTo IncompleteWith
          End If
InsertErrorMEssage:
...
IncompleteWithError:
  ...
  If Embedded = -1 Then
    ArrRoutine(GetEndOfRoutine(ArrRoutine)) = Marker(ArrRoutine(GetEndOfRoutine(ArrRoutine)), WARNING_MSG & "Purify With had a problem with this procedure; possible empty With structure.", MBefore, UpDated)
   Else
    ArrRoutine(GetEndOfRoutine(ArrRoutine)) = Marker("End With" & vbNewLine & _
     ArrRoutine(GetEndOfRoutine(ArrRoutine)), WSPmsg, MBefore, UpDated)
...
':)Code Fixer V2.9.6 (9/02/2005 3:11:15 AM) 1 + 1123 = 1124 Lines Thanks Ulli for inspiration and lots of code.
