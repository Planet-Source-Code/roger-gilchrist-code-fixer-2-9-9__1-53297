                      ...
                      If Len(ArrOld(J)) Then
                        ...
                        For K = LBound(ArrNew) To UBound(ArrNew)
                          If ArrNew(K) <> strFind Then
                            If MultiRight(ArrNew(K), False, strFind) Then
              ...
              For J = 0 To UBound(ArrParam)
                ...
                If LenB(Trim$(ArrParam(J))) Then
                  ...
                  If Get_As_Pos(ArrParamTest(J)) > 0 Then
                    'If Not StandardTypeExists(GetType(ArrParamTest(J))) Then
          ...
          If Not JustACommentOrBlank(L_CodeLine) Then
            If InstrAtPosition(L_CodeLine, "Function", ipLeftOr2ndOr3rd) Then
              ...
              If ExtractCode(L_CodeLine) Then
                If MultiRight(Trim$(strTest), True, RBracket) And Not IsArrayReturningFunction(strTest) Then
  ...
  If UsingDefTypes Then
   ...
   Else
    ...
    For I = LBound(arrModule) + 1 To UBound(arrModule) - 1
      If MultiLeft(arrModule(I), True, FName) Then
        ...
        If UBound(arrLine) > 0 Then
          If arrLine(1) = "=" Then
           ...
           ElseIf UBound(arrLine) > 1 Then
            'v2.9.6 possible bug?
            If InQSortArray(ExistingDimArray, arrLine(I)) Then
  ...
  If bEventDescExists Then
    For I = LBound(EventDesc) To UBound(EventDesc)
      If EventDesc(I).EScope = "Public" Then
        If MultiRight(strTest, True, EventDesc(I).EName) Then
          isUserEvent = True
          Exit For
        End If
                ...
                Select Case FixData(ParameterDead_CXX).FixLevel
                 Case CommentOnly
                  If InQSortArray(ImplementsArray, strCmpName) Then
                    ArrRoutine(TopOfRoutine) = Marker(L_CodeLine, WARNING_MSG & "Unused Parameter" & IIf(InStr(strMsg, CommaSpace), "s", vbNullString) & strInSQuotes(strMsg, True) & vbNewLine & _
                     "This is an Interface class; Parameters must be left in place.", MAfter, UpDated)
                   Else
                    ArrRoutine(TopOfRoutine) = Marker(L_CodeLine, WARNING_MSG & "Unused Parameter" & IIf(InStr(strMsg, CommaSpace), "s", vbNullString) & strInSQuotes(strMsg, True) & "could be removed.", MAfter, UpDated)
                  End If
          ...
          If Not JustACommentOrBlank(Arr(I)) Then
            ...
            For J = 0 To UBound(ArrParam)
              If Len(ArrParam(J)) Then
                ...
                End If
                If MultiLeft(L_CodeLine, True, ArrParam(J)) Then
                  ArrParamFull(J) = vbNullString
                  ArrParam(J) = vbNullString
                End If
              End If
...
Private Function WriteType(arrModule As Variant, _
  ...
  If MultiLeft(arrModule(lineMod), True, strType & " " & arrLine(2)) Then
...
':)Code Fixer V2.9.6 (9/02/2005 3:00:45 AM) 21 + 1591 = 1612 Lines Thanks Ulli for inspiration and lots of code.
