...
Public Function GetErrorTrapLevel() As ErrLevel
  ...
  A = QueryValue(VBSettings, "BreakOnAllErrors") * 2
  B = QueryValue(VBSettings, "BreakOnServerErrors")
  GetErrorTrapLevel = A + B
...
''
''Public Sub Safe_DoEvents()
'''only does doevents if there has been input
''If GetInputState <> 0 Then
''DoEvents
''End If
''If fCodeFix.WindowState = vbMinimized Then
'''allow interaction to bring back from taskbar
''DoEvents
''End If
''End Sub
''

':)Code Fixer V2.9.6 (9/02/2005 2:55:22 AM) 30 + 172 = 202 Lines Thanks Ulli for inspiration and lots of code.
