          ...
          If Not JustACommentOrBlank(L_CodeLine) Then
            ...
            If dofix(ModuleNumber, DetectDangerousCode_CXX) Then
              For J = LBound(DangerousCodeArray) To UBound(DangerousCodeArray)
                'If InstrAtPosition(L_Codeline, DangerousCodeArray(J), IpLeft) Then
                If WordInString(L_CodeLine, 1) = DangerousCodeArray(J) Then
                  ArrRoutine(RLine) = Marker(ArrRoutine(RLine), RISK_MSG & " Risky Code " & DangerousCodeArray(J) & RiskMessage(DangerousCodeLevelArray(J)), MAfter, UpDated)
                  AddNfix DetectDangerousCode_CXX
                End If
              Next J
              For J = LBound(DangerousScriptArray) To UBound(DangerousScriptArray)
                'If InstrAtPosition(L_Codeline, DangerousCodeArray(J), IpLeft) Then
                If InStrCode(LCase$(L_CodeLine), DangerousScriptArray(J)) Then
                  ArrRoutine(RLine) = Marker(ArrRoutine(RLine), RISK_MSG & " Risky Code " & DangerousScriptArray(J) & RiskMessage(DangerousScriptLevelArray(J)), MAfter, UpDated)
                  AddNfix DetectDangerousCode_CXX
                End If
              Next J
            End If
            If dofix(ModuleNumber, DetectDangerousReference_CXX) Then
              For J = LBound(DangerousReferenceArray) To UBound(DangerousReferenceArray)
                'If InstrAtPosition(L_Codeline, DangerousReferenceArray(J), IpLeft) Then
                If WordInString(L_CodeLine, 1) = DangerousReferenceArray(J) Then
                  ArrRoutine(RLine) = Marker(ArrRoutine(RLine), RISK_MSG & " Reference to " & DangerousReferenceArray(J) & RiskMessage(DangerousReferenceArray(J)), MAfter, UpDated)
                  AddNfix DetectDangerousAPICode_CXX
                End If
              Next J
            End If
            If dofix(ModuleNumber, DetectDangerousString_CXX) Then
              For J = LBound(DangerousStringArray) To UBound(DangerousStringArray)
                'If InStr(L_Codeline, LCase$(DangerousStringArray(J))) Then
                If LCase$(WordInString(L_CodeLine, 1)) = LCase$(DangerousStringArray(J)) Then
                  ArrRoutine(RLine) = Marker(ArrRoutine(RLine), RISK_MSG & "'" & DangerousStringArray(J) & "' could be used dangerously. Check that the code is safe.", MAfter, UpDated)
                  AddNfix DetectDangerousAPICode_CXX
                End If
              Next J
            End If
...
Public Sub FindAndComment(ByVal strFind As String, _
                          ...
                          Optional ByVal bPattern As Boolean = True)
  ...
  For Each Proj In VBInstance.VBProjects
    For Each Comp In Proj.VBComponents
      If SafeCompToProcess(Comp, CurCompCount) Then
        ...
        Scol = 1
        With Comp
          Do While .CodeModule.Find(strFind, StartLine, Scol, -1, -1, False, False, bPattern)
            strFound = .CodeModule.Lines(StartLine, 1)
            MemberMessage "", StartLine, .CodeModule.CountOfLines
            If InCode(strFound, Scol) Then
  ...
  For I = LBound(arrR) To UBound(arrR)
    If Not JustACommentOrBlank(arrR(I)) Then
      ...
      If InStrCode(arrR(I), " Resume ") Or Left$(arrR(I), 7) = "Resume " Then
        ...
        End If
      End If
      If InStrCode(arrR(I), " GoSub ") Or MultiLeft(arrR(I), True, "GoSub ") Then
...
Private Sub GetStructureLevel(ByVal VarLine As Variant, _
  ...
  For I = LBound(arrLine) To UBound(arrLine)
    ...
    If InstrAtPositionArray(strTest, IpLeft, True, strStart) Then
      If strStart = "If" Then
    ...
    For J = LBound(arrEnd) To UBound(arrEnd)
      If InstrAtPositionArray(strTest, IpLeft, True, arrEnd(J)) Then
        Slvl = Slvl - 1
        If UBound(arrEnd) > 0 Then
          Slvl = Slvl + 1
        End If
  ...
  For I = LBound(arrTarget) To UBound(arrTarget)
    ...
    If arrTarget(I) > GTData.LineNo Then
      ...
      For J = TopScan To EndScan
        ...
        If MultiLeft(arrR(J), True, strTrigger) Then
                  ...
                  Do
                    ...
                    If Not JustACommentOrBlank(L_Codeline2) Then
                      ...
                      If InstrAtPositionArray(L_Codeline2, IpLeft, True, "Exit Do", "Exit For", "GoTo") Then
                        If MultiLeft(L_Codeline2, True, "Exit Do") Then
                          strTest = "Loop"
                         ElseIf MultiLeft(L_Codeline2, True, "Exit For") Then
                          strTest = "Next"
                         Else
                        ...
                        Do
                          ...
                          If MultiLeft(L_Codeline3, True, strTest) Then
                      ...
                      Select Case FixData(DetectObsoleteCodeStructure_CXX).FixLevel
                       ...
                       Case FixAndComment
                        ...
                        For K = Member To UBound(ArrRoutine)
                          If MultiLeft(ArrRoutine(K), True, "Wend ") Then
        ...
        If SafeCompToProcess(Comp, CurCompCount) Then
          ...
          With Comp
            If .CodeModule.CountOfLines Then
              WorkingMessage "Poor Control name", 5, NumFixes
              BadControlName .CodeModule, Comp
              WorkingMessage "Detect Obsolete Code Structures", 6, NumFixes
              ObsoleteCode .CodeModule
              WorkingMessage "Risky API/Code/Ref/Str/Hard-Path", 7, NumFixes
              DangerousCoding .CodeModule
              WorkingMessage "Static To Private", 8, NumFixes
              StaticToPrivate .CodeModule
              WorkingMessage "Duplicate Public Procedures", 9, NumFixes
              DuplicatePublicProcedures .CodeModule, Comp
              WorkingMessage "Illegal With Exits", 10, NumFixes
              IllegalWithExit .CodeModule
              WorkingMessage "GoTo evaluation", 11, NumFixes
              GoToCommentry .CodeModule
              WorkingMessage "ElseIf Evaluate", 12, NumFixes
              ElseIfEvaluator .CodeModule
              WorkingMessage "Large Procedures", 13, NumFixes
    ...
    For I = LBound(arrTarget) To UBound(arrTarget)
      ...
      For J = TopScan To EndScan
        ...
        If strBegin = "With" Then
          If MultiLeft(arrR(J), True, strEnd) Then
            Depth = Depth - 1
          End If
         Else
...
':)Code Fixer V2.9.6 (9/02/2005 3:05:57 AM) 37 + 1340 = 1377 Lines Thanks Ulli for inspiration and lots of code.
