...
Private Function AlreadyTrapped(ByVal arrR As Variant) As Boolean
  ...
  For I = LBound(arrR) To UBound(arrR)
    If MultiLeft(arrR(I), True, TrapTop) Then
      AlreadyTrapped = True
      Exit For 'unction
    End If
  Next I
...
Private Function DetectNonBugTrapErrorHandling(arrR As Variant) As Boolean
  ...
  For I = LBound(arrR) To UBound(arrR)
    ...
    If IsOnErrorCode(arrR(I), 1) Then
      'If InStr(arrR(I), "On Error Goto ") Then
      strTest = WordAfter(arrR(I), "GoTo")
      If Not ArrayMember(strTest, "0", "BugTrap") Then
        For J = I To UBound(arrR)
          If MultiLeft(Trim$(arrR(J)), True, strTest & ":") Then
            For K = J To I Step -1
              If MultiLeft(Trim$(arrR(K)), True, "Exit " & ProcKind) Then
...
Private Function StrBugTrapSub(Optional bPrivate As Boolean = False, _
  ...
  StrBugTrapSub = vbNewLine & _
   ...
   " Open  " & StrOutputPath & " For Append As #FFile" & vbNewLine & _
...
':)Code Fixer V2.9.6 (9/02/2005 3:09:55 AM) 19 + 329 = 348 Lines Thanks Ulli for inspiration and lots of code.
