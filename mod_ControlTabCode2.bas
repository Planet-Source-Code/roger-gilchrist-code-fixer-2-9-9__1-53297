        ...
        If CntrlDesc(lngWorkingDesc).CDIndex = "-1" Then
          ...
          If InQSortArray(arrTarget, CntrlDesc(lngWorkingDesc).CDClass) Then
            If CntrlDesc(lngWorkingDesc).CDClass = "Menu" Then
              If ArrayMember(CntrlDesc(lngWorkingDesc).CDBadType, BNStructural, BNClass, BNCommand, BNMultiForm) Then
          ...
          If Not Targetcontrol Is Nothing Then
            ...
            If UBound(ArrayCntrls) = 0 Then
              With Targetcontrol
                ...
                RenameControlsStandard strNewName & strInBrackets(.Properties("index").Value), strNewName
...
Public Sub FixControlCase(strNewName As String)
  ...
  For I = LBound(StandardCtrPrefix) To UBound(StandardCtrPrefix)
    If MultiLeft(strNewName, False, StandardCtrPrefix(I)) Then
      ...
      If LenB(Comp.Name) Then
        ...
        If LModule.Find(strError, StartLine, 1, -1, -1, bWholeWordonly, bCaseSensitive, False) Then
          ...
          Do
            ...
            lngPrevFindLine = StartLine
          Loop While GetWholeCaseMatchCodeLine(Proj.Name, Comp.Name, strError, L_CodeLine, StartLine)
        End If
      End If
    Next Comp
  Next Proj
...
Public Sub NoCodeCommentry(LItem As ListItem, _
  ...
  With LItem
    If CntrlDesc(I).CDBadType > 0 Then
      ...
      'ElseIf bIgnoreSingleton And CntrlDesc(I).CDXPFrameBug <> 0 Then
...
':)Code Fixer V2.9.6 (9/02/2005 2:56:36 AM) 9 + 917 = 926 Lines Thanks Ulli for inspiration and lots of code.
