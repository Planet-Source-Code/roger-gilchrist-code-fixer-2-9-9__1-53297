              ...
              If Val(Words(WordIndex + 2)) >= -IntegerLimit - 1 Then
                If Val(Words(WordIndex + 2)) <= IntegerLimit Then
                  If Val(Words(WordIndex + 2)) = Int(Val(Words(WordIndex + 2))) Then
                    DefConstType = Not (TypeSuffixExists(Words(WordIndex + 2)) Or MultiLeft(Words(WordIndex + 2), True, Quote) Or (MultiLeft(Words(WordIndex + 2), True, "&") And Len(Words(WordIndex + 2)) > 6))
                  End If
                End If
              End If
            End If
...
Public Sub FormatAll()
  ...
  With frm_CodeFixer
    ...
    If VBInstance.VBProjects.VBE.ActiveVBProject.VBComponents.Count Then
      ...
      SectionMessage "Build Control Arrays", 1 / SectionTotal
      If Not Control_Engine Then
        bAbortComplete = True
        bFixing = False
        GoTo SafeExit
      End If
      SectionMessage "Clean Up", 2 / SectionTotal
      CleanUp_Engine
      SectionMessage "XP && Frame test", 3 / SectionTotal
      XPManifestFrameWarning
      SectionMessage "Declaration Section", 4 / SectionTotal
      Declaration_Engine
      SectionMessage "Build Data Arrays", 5 / SectionTotal
      Generate_Publics
      SectionMessage "Restructure", 6 / SectionTotal
      ReStructure_Engine
      SectionMessage "Parameters", 7 / SectionTotal
      Parameter_Engine
      SectionMessage "Local variables", 8 / SectionTotal
      Dim_Engine
      SectionMessage "Suggest", 9 / SectionTotal
      Suggest_Engine
      SectionMessage "Unused", 10 / SectionTotal
      Unused_Engine
      SectionMessage "Layout", 11 / SectionTotal
      FormatDo
      If bAborting Then
        AbortFormatting
      End If
                   ...
                   Else
                    ...
                    Select Case Words(LastWordIndex)
                     Case "sub", "function", "property", "for", "do"
                      .ReplaceLine LineIndex, .Lines(LineIndex, 1) & IIf(MultiRight(.Lines(LineIndex, 1), True, "Expand Structure"), ToDo2a, vbNullString)
                    End Select
                  End If
                     ...
                     Case "with"
                      ...
                      If Xcheck(XStructCom) Then
                       ...
                       Else
                        ...
                        If MultiRight(.Lines(LineIndex, 1), True, Tmpstring1) = 0 Then
    ...
    For Each Proj In VBInstance.VBProjects
      For Each Comp In Proj.VBComponents
        If SafeCompToProcess(Comp, CurCompCount) Then
          ...
          WorkingMessage "Indenting", 1, 2
          FormatCode cMod
          WorkingMessage "Line continuation Layout", 2, 2
          ...
          If bPaulCatonSubClasUsed Then
            If strPaulCatonSubClasCompName = cMod.Parent.Name Then
              If strPaulCatonSubClasPRocName = GetFirstProcedureName(cMod) Then
...
Private Sub MoveUnusedRoutines(cMod As CodeModule)
  ...
  With cMod
    ...
    For I = 1 To MaxFactor
      If MultiLeft(ArrCode(I), True, MoveableComment) Then
...
Private Sub PopPush(cMod As CodeModule, _
  ...
  With cMod
    If Condition Then
      Tmpstring1 = PopWord()
      If Xcheck(XStructCom) Then
        'If InStr(1, .Lines(LLine, 1), Tmpstring1, vbTextCompare) = 0 Then
        'v2.2.0 maybe fixes error with multiple messages being attached
        If MultiRight(.Lines(LLine, 1), True, Tmpstring1) = 0 Then
...
Public Function TypeSuffixExistsAnywhere(ByVal strSearch As String) As Boolean
  ...
  For I = 1 To 6
    If InCode(strSearch, InStr(strSearch, Mid$("%&@!#$", I, 1))) Then
...
':)Code Fixer V2.9.6 (9/02/2005 3:06:34 AM) 84 + 1670 = 1754 Lines Thanks Ulli for inspiration and lots of code.
