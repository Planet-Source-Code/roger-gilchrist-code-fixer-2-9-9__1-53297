            ...
            Select Case AtLocation
             ...
             Case IpLeft
              If varSearch = varFind Then
               ...
               ElseIf MultiLeft(varSearch, True, varFind) Then
                If WholeWord Then
             ...
             Case IpRight
              If varSearch = varFind Then
                InstrAtPosition = True
               ElseIf MultiRight(varSearch, True, varFind) Then
                If WholeWord Then
              ...
              If WholeWord Then
               ...
               Else
                If SizeOfSearch > 0 Then
                  TmpA(0) = vbNullString
                  InstrAtPosition = MultiLeft(Trim$(Join(TmpA)), True, varFind)
                End If
              End If
             Case ip3rd
              If WholeWord Then
        ...
        If InstrAtPosition(strSearch, findMember, AtLocation, WholeWord) Then
          If WholeWord Then
            If InStrWholeWord(strSearch, findMember) Then
              'v2.3.2 safer test
              'If IsRealWord(strSearch, CStr(findMember)) Then
  ...
  If InStr(varSearch, varFind) Then
    If WholeWord Then
      If varSearch = varFind Then
        bFound = True
        InStrWholeWord = 1
       ElseIf MultiLeft(varSearch, True, varFind) And IsPunct(Mid$(varSearch, 1 + Len(varFind), 1)) Then
        InStrWholeWord = IIf(IsPunct(Mid$(varSearch, InStr(varSearch, varFind) + Len(varFind), 1)), 1, 0)
       ElseIf MultiRight(varSearch, True, varFind) And IsPunct(Mid$(varSearch, Len(varSearch) - Len(varFind), 1)) Then
...
Public Sub LoadUserSettings()
  ...
  With frm_FindSettings 'CodeFixer
    ...
    .OptElseIndent(lngCaseIndent) = True
...
Public Function ReplaceArray2(ByVal strInput As String, _
                              ParamArray VRep() As Variant) As String
  ...
  'this function replaces multiple calls to Replace
  ReplaceArray2 = strInput
  For I = LBound(VRep) To UBound(VRep) Step 2
    If Len(VRep(I)) Then
      'v2.4.4 improved for multiple hits
      ReplaceArray2 = Safe_Replace(ReplaceArray2, VRep(I), VRep(I + 1))
    End If
  Next I
...
':)Code Fixer V2.9.6 (9/02/2005 2:58:29 AM) 43 + 1873 = 1916 Lines Thanks Ulli for inspiration and lots of code.
