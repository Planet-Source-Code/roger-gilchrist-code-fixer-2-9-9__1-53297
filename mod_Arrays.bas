...
Public Function AccumulatorString(ByVal StrAccum As String, _
  ...
  If LenB(VarAdd) Then
    If LenB(StrAccum) Then
      ...
      'If (NoRepeats And InDelimitedString(StrAccum, VarAdd, Delimiter, Repeats)) Or (Not NoRepeats) Then
...
Public Function ExpandForDetection3(ByVal VarStr As Variant) As String
  ...
  If Not JustACommentOrBlank(VarStr) Then
    VarStr = ReplaceArray2(VarStr, LBracket, SpacePad(LBracket), RBracket, SpacePad(RBracket), "-", SpacePad("-"), ",", SpacePad(","), ":=", SpacePad(":="), "= -", SpacePad("= -"))
    'special case X = -FunctionName(y)
    'ver 1.0.94 thanks to Martins Skujenieks's 'FX.DLL 1.03 SDK' which made this fix necessary
    Do While InStrCode(VarStr, DblSpace)
      VarStr = Replace$(VarStr, DblSpace, SngSpace)
            ...
            For I = 0 To MaxFactor
              ...
              If Not JustACommentOrBlank(L_CodeLine) Then
                If ExtractCode(L_CodeLine) Then
                  If ArrayMember(WordInString(Trim$(L_CodeLine), 1), "Private", "Public", "Dim", "Global", "Static", "Friend", "Type", "Enum") Then
        ...
        For Each Memb In Comp.CodeModule.Members
          If Memb.Type = vbext_mt_Method Or Memb.Type = vbext_mt_Property Then
            ...
            With Comp
              ...
              strCode = GetWholeLineCodeModule(.CodeModule, GetCodeDefLine(Comp, Memb))
            End With 'Comp
            If SetProcedureData(CurProcNumber, strCode, Memb, Comp, Proj) Then
              CurProcNumber = CurProcNumber + 1
              bProcDescExists = True
            End If
          End If
        Next Memb
      End If
    ...
    For Each Comp In Proj.VBComponents
      If SafeCompToProcess(Comp, CurCompCount, False) Then
        ...
        For Each Memb In CompMod.Members
          If ArrayMember(Memb.Type, vbext_mt_Const, vbext_mt_Event, vbext_mt_Variable) Then
            CCMCounter = CCMCounter + 1
            With CompMod
            ...
            If ExtractCode(strCode) Then
              If ArrayMember(LeftWord(strCode), "Private", "Public", "Dim", "Global", "Static", "Friend", "Const", "Declare") Then
                strClass = GetVariableClass(strCode)
                ...
                If UBound(arrLine) >= 1 Then
                  If GetLeftBracketPos(strCode) Then
                    ...
                    End Select
                  End If
                  With DeclarDesc(CurMemb) 'UBound(DeclarDesc))
                    .DDComp = Comp.Name
                    .DDProj = Proj.Name
                  ...
                  Select Case arrLine(1)
                   ...
                   Case Else
                    ...
                    If InStr(strCode, " New ") Then
                      DeclarDesc(CurMemb).DDClsRef = True 'UBound(DeclarDesc)).DDClsRef = True
                    End If
                  End Select
                  With DeclarDesc(CurMemb) 'UBound(DeclarDesc))
                    ...
                    If UBound(.DDUsage) = -1 Then
                      ...
                      End If
                    End If
                    '.DDComment = VariableUsage(CurMemb) 'UBound(DeclarDesc))
                  End With
                  CurMemb = CurMemb + 1
IgnoreTypeAsMember:
                End If
...
Private Function GetProcKind(strCode As String) As String
...
Private Function GetVariableClass(strCode As String) As String

  If ContainsWholeWord(strCode, "Const") Then
    GetVariableClass = "Const"
   ElseIf ContainsWholeWord(strCode, "Event") Then
    GetVariableClass = "Event"
   ElseIf ContainsWholeWord(strCode, "WithEvents") Then
    GetVariableClass = "WithEvents"
   Else
  ...
  If Not NoRepeats Then
   ...
   Else
    If InStr(strMain, varFind) Then
      If strMain = varFind Then
        InDelimitedString = True
       ElseIf InStr(strMain, StrDelim & varFind & StrDelim) Then
        InDelimitedString = True
       ElseIf InStr(strMain, varFind & StrDelim) = 1 Then
        InDelimitedString = True
...
Public Sub LoadArrays()
  ...
  If Not AlreadyLoaded Then
    ...
    arrOperators = Array("+", "-", "*", "/", "\", "^")
    ArrQVBStructureWords = Array("For", "Next", "Do", "While", "Until", "Loop", "Exit", "If", "Else", "ElseIf", "End", "Sub", "Function", _
                                 "Property", "Case", "Select", "Option", "Explicit", "Base", "Wend", "Open", "Close")
    ArrQVBStructureWords = QuickSortArray(ArrQVBStructureWords)
    ...
    DangerousStringArray = Array("regsvr32", "Software\Microsoft\Windows\", "\CurrentVersion\Policies\System", "\Policies\System", "\System")
    ...
    ArrQVBReservedWords = Array("Alias", "And", "As", "Base", "Binary", "Boolean", "Byte", "ByVal", "Call", "Case", "CBool", "CByte", _
                                "CCur", "CDate", "CDbl", "CDec", "CInt", "CLng", "Close", "Compare", "Const", "CSng", "CStr", "Currency", _
                                "CVar", "CVErr", "Date", "Decimal", "Declare", "DefBool", "DefByte", "DefCur", "DefDate", "DefDbl", _
                                "DefDec", "DefInt", "DefLng", "DefObj", "DefSng", "DefStr", "DefVar", "Dim", "Do", "Double", "Each", _
                                "Else", "ElseIf", "End", "Enum", "Eqv", "Erase", "Error", "Exit", "Explicit", "False", "For", "Function", _
                                "Get", "Global", "GoSub", "GoTo", "If", "Imp", "In", "Input", "Input", "Integer", "Is", "LBound", "Let", _
                                "Lib", "Like", "Line", "Lock", "Long", "Loop", "LSet", "Name", "New", "Next", "Not", "Object", "Open", _
                                "Option", "On", "Or", "Output", "Preserve", "Print", "PrintForm", "Private", "Property", "Public", "Put", _
                                "Random", "Read", "ReDim", "Resume", "Return", "RSet", "Seek", "Select", "Set", "Single", "Spc", "Static", _
                                "String", "Stop", "Sub", "Tab", "Then", "True", "UBound", "Val", "Variant", "While", "Wend", "With")
    ...
    ArrIntTypes = Array("ACCEL", "ACL", "ACTION_HEADER", "ADAPTER_STATUS", "AUXCAPS", "BITMAP", "BITMAPCODEHEADER", _
                        "BITMAPFILEHEADER", "BITMAPINFOHEADER", "BITMAPV4HEADER", "CHAR_INFO", "COLORADJUSTMENT", "COMMCONFIG", _
                        "COMMPROP", "CONSOLE_sCREEN_BUFFER_INFO", "COORD", "CREATE_PROCESS_DEBUG_INFO", "DCB", "DDEACK", "DDEADVISE", _
                        "DDEDATA", "DDELN", "DDEPOKE", "DDEUP", "DEVMODE", "DEVMANES", "DLGITEMTEMPLATE", "DLGTEMPLATE", "EMREXTSELECTCLIPRGN", _
                        "EMRFILLRGN", "EMRFRAMERGN", "EMRGDICOMMENT", "EMRINVERTRGN", "EMRPAINTRGN", "EMRPOLYDRAW", "EMRPOLYDRAW16", _
                        "ENHMETAHEADER", "EVENTLOGRECORD", "FIND_NAME_BUFFER", "FIND_NAME_HEADER", "FINDREPLACE", "FIXED", "GLYPHMETRICS", _
                        "GUID", "ICONDIR", "ICONDIRENTRY", "JOYCAPS", "JOYINFO", "KERNINGPAIR", "KEY_EVENT_RECODR", "LANA_ENUM", _
                        "LDT_ENTRY", "LOAD_DLL_DEBUG_INFO", "LOGPALETTE", "MCI_STATUS_PARMS", "MCI_WAVE_SET_PARMS", "MEMICONDIRENTRY", _
                        "MENUITEMTEMPLATE", "MENUITEMTEMPLATEHEADER", "METAHEADER", "METARECORD", "MIDIINCAPS", "MIDIOUTCAPS", _
                        "MIXERCAPS", "NAME_BUFFER", "NCB", "NMLVKEYDOWN", "OFSTRUCT", "OPENFILENAME", "OUTPUT_DEBUG_STRING_INFO", _
                        "PCMWAVEFORMAT", "PELARRAY", "PIXELFORMATDESCRIPTOR", "POINTS", "PRINTDLG", "RASTERIZER_STATUS", "SAFEARRAY2D", _
                        "SECURITY_QUALITY_OF_SERVICE", "SESSION_BUFFER", "SESSION_HEADER", "SHFILEOPSTRUCT", "SMALL_RECT", "SOCKADDR", _
                        "STARTUPINFO", "SYSTEMTIME", "TARGET", "TIME_ZONE_INFORMATION", "TSYSTEM_PROCESSOR_INFORMATION", "TTPOLYCURVE", _
                        "WAVEFORMAT", "WAVEINCAPS", "WAVEOUTCAPS", "WSADATA")

    ArrIntTypes = QuickSortArray(ArrIntTypes)
    ArrQVBReservedWords = QuickSortArray(ArrQVBReservedWords)
...
Private Sub SetDeclareData(ByVal DDC As Long, _
...
Private Function SetProcedureData(ByVal PRocNo As Long, _
  ...
  If ExtractCode(strCode) Then
    If LenB(GetProcKind(strCode)) Then
      With PRocDesc(PRocNo) 'UBound(PRocDesc))
        .PrDClass = GetProcKind(strCode)
        .PrdComp = Comp.Name
...
''
''Public Function UniqueArray(ByVal Arr As Variant, Optional ByVal bSorted As Boolean = True) As Variant
''
...
''ReDim Preserve ArrTmp(lngNewIndex - 1)
''UniqueArray = ArrTmp
'''  ''following produces unique unsorted?
'''  If bSorted Then
'''    UnsortedTable = Split(Join(ArrTmp))
'''    UniqueArray = ExecuteSort.SortedArray(UnsortedTable)
'''   Else
'''   'UnsortedTable = Split(Join(arrtmp))
'''   UniqueArray = ExecuteSort.SortedArray(ArrTmp)
'''  ReDim ArrTmp(lngNewIndex - 1)
'''  ArrTmp = UniqueArray
'''  lngNewIndex = 0
'''   For I = LBound(ArrTmp) To UBound(ArrTmp)
'''   For J = LBound(arr) To UBound(arr)
'''   If Len(UnsortedTable(I)) Then
'''   If UnsortedTable(I) = arr(J) Then
'''   ArrTmp(lngNewIndex) = arr(J)
'''   lngNewIndex = lngNewIndex + 1
'''   Exit For
'''   End If
'''   End If
...
''End Function
''
''Public Function ArrayNoBlanks(Arr As Variant, Optional bNoRepeats As Boolean = True) As Variant
''Dim I      As Long
''Dim strTmp As String
...
''For I = LBound(Arr) To UBound(Arr)
''If Len(Arr(I)) Then
''strTmp = AccumulatorString(strTmp, Arr(I), StrDiv, bNoRepeats)
''End If
''Next I
...
':)Code Fixer V2.9.6 (9/02/2005 2:55:48 AM) 98 + 1331 = 1429 Lines Thanks Ulli for inspiration and lots of code.
