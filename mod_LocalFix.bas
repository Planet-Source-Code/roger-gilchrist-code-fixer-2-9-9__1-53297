...
Public Sub CtrlDefPropFast()
  ...
  For I = LBound(CntrlDesc) To UBound(CntrlDesc)
    ...
    If Not ArrayMember(CntrlDesc(I).CDName, arrDone) Then
  ...
  Do While Cmp.CodeModule.Find(StrSearchFor, Sline, Scol, ELine, Ecol, False, False, True)
    ...
    If Not JustACommentOrBlankOrDimLine(L_CodeLine) Then
      If Not IsDimLine(L_CodeLine) Then
        ...
        If Not PotentialControlDefault(Cmp, tmpB) Then
          GoTo NoSuspectCode  ' no suspect code; so get next line
        End If
        CompactIndexes tmpB
             ...
             Else
              ...
              If tmpB(J) <> ProcName Then
                ...
                If InQSortArray(ExistingDimArray, tmpB(J)) Then
                  ...
                  'If DimLine > -1 Then
                  '  If InStr(TmpA(DimLine), WARNING_MSG & "The variable '" & tmpB(J) & "'") = 0 Then
                  '   'only comment once (while still in array the msg is part of the line
                  '    TmpA(DimLine) = SmartMarker(TmpA, DimLine, WARNING_MSG & "The variable '" & tmpB(J) & "' has the same name as a Control" & SuggestNewName(tmpB(J), TmpA(DimLine)), MAfter)
                  '  End If
                  'End If
                  'Exit For
               ...
               ElseIf IsControlProperty(tmpB(J)) And Not IsVBControlRoutine(ProcName) Then
                ...
                'If DimLine > -1 Then
                '  If InStr(TmpA(DimLine), WARNING_MSG & "The variable '" & tmpB(J) & "'") = 0 Then
                '    TmpA(DimLine) = SmartMarker(TmpA, DimLine, WARNING_MSG & "The variable '" & tmpB(J) & "' has the same name as a Control Property" & SuggestNewName(tmpB(J), TmpA(DimLine)), MAfter)
                '  End If
                'End If
          ...
          If CompMod.Members.Count Then
            ...
            For I = 1 To UBound(arrMembers)
              ...
              If LenB(arrMembers(I)) Then
                ...
                If InstrArray(strRoutine, "Dim ", "Static ", "Const ") Then
                ...
                End If
                ' strRoutine = DimControlDefaultProp(CurCompCount, strRoutine, Comp)
                If InstrArray(strRoutine, "Dim ", "Static ", "Const ") Then
                ...
                If InstrArray(strRoutine, "Dim ", "Static ", "Const ") Then
                  'If InstrAtPositionArray(strRoutine, ipAny, True, "Dim", "Static", "Const") Then
                  ...
                  '''strRoutine = DimSort(strRoutine)
                  'FIXME under development DO NOT uncomment it'll kill your code
...
SkipErrDetectionFunction:
              If AssignedOnly Then
                ...
                End Select
              End If
              'If Not SuspicoiusSelfRefererence And UnusedAPIReturn And UnusedFunctionReturn And UnusedFunctionNoParameters And UnusedVBFunction Then
    ...
    If Left$(strTmp, 6) = "Const " Then
     ...
     ElseIf Left$(strTmp, 6) = "ReDim " Then
      If getReDimType(WordInString(ExpandForDetection(strTmp), 2), VarRoutine, AsType) Then
        'VarRoutine(DimLine) = VarRoutine(DimLine) & " As " & AsType
     ...
     Else
      ...
      If Not ReturnCannotDoItMsg Then
        If LenB(AsType) = 0 Then
          Exit Sub
        End If
      End If
      FoundIt = True
                ...
                Else
                  If InStrWholeWord(ArrProc(J), strTest) Then
                    If InStructure(LoopStruct, ArrProc, J, lngJunk, lngJunk) Or InStructure(ForStruct, ArrProc, J, lngJunk, lngJunk) Then
                      ...
                      Exit For
                    Else
                      If MultiLeft(ArrProc(J), True, strTest & " = ") Then
...
Private Function GetDeclarationIndexing(ByVal strTest As String, _
...
Private Function getReDimType(ByVal strFind As String, _
  ...
  For I = LBound(VarRoutine) To UBound(VarRoutine)
    ...
    End If
    'Local Dim line
    If MultiLeft(VarRoutine(I), True, "Dim " & strFind) Then
      ...
      End If
    End If
    '    If InStr(VarRoutine(I), "Dim " & strFind) Then
    '      strType = WordAfter(VarRoutine(I), " As ")
    '      If Len(strType) Then
    '        getReDimType = True
    '        Exit For
...
Public Function PotentialControlDefault(Comp As VBComponent, _
                                        ByVal arrTmp As Variant) As Boolean
  ...
  If ArrayMember(arrTmp(0), "With", "Load", "Set", "Unload", "For", "Next", "RaiseEvent", "End") Then
   ...
   ElseIf ArrayMember(arrTmp(0), "Name") Then
   ...
   Else
    For J = LBound(arrTmp) To UBound(arrTmp)
      If Not IsPunct(CStr(arrTmp(J))) Then
          ...
          If Not JustACommentOrBlankOrDimLine(L_CodeLine) Then
            If ExtractCode(L_CodeLine, strComment) Then
              For J = LBound(TypeSuffixArray) To UBound(TypeSuffixArray)
                If Not ArrayMember(TypeSuffixArray(J), "$", "&") Then
              ...
              If InCode(L_CodeLine, InStr(L_CodeLine, "$")) Then
                ...
                For J = LBound(arrLine) To UBound(arrLine)
                  If Right$(arrLine(J), 1) = "$" Then
                    If InCode(L_CodeLine, InStr(L_CodeLine, arrLine(J))) Then 'ignore in literals
...
''Public Function GetInsertionPointArray(arr As Variant, Lnum As Long) As String
...
''StrLine = arr(Lnum)
''Loop
''GetInsertionPointArray = Lnum
''End Function
''
'  Public Function DimSort(ByVal strA As String) As String
'Dim ArrProc       As Variant
'Dim I             As Long
'Dim J             As Long
'Dim DimCount      As Long
...
'Dim MaxFactor     As Long
'Dim Hit           As Boolean
'Dim arrDimBlock   As Variant
'Dim arrDimComment As Variant
'Dim BlockEnd      As Long
'Dim ArrSorts      As Variant
'Dim lngTop        As Long
'  EOLOffSet = 0
'  AsOffSet = 0
'  ArrProc = Split(strA, vbNewLine)
'  MaxFactor = UBound(ArrProc)
'  ReDim arrDimBlock(UBound(ArrProc)) As Variant
'  If MaxFactor > 0 Then
'    For I = GetProcCodeLineOfRoutine(ArrProc) + 1 To MaxFactor
'      WorkingMessage "Dim Sort", I, MaxFactor
'      L_CodeLine = ArrProc(I)
'      If Not JustACommentOrBlank(L_CodeLine) Then
'        If IsDimLine(L_CodeLine, False) Then
'          arrDimBlock(I) = ArrProc(I)
'          DimCount = DimCount + 1
'          BlockEnd = I
'        Else
'          Exit For
...
'    Next I
'    If DimCount > 1 Then
'      arrDimBlock = NoBlanksArray(arrDimBlock)
'      ReDim arrDimComment(UBound(arrDimBlock))
'      For I = 0 To UBound(arrDimBlock)
'        L_CodeLine = arrDimBlock(I)
'        If IsDimLine(L_CodeLine, False) Then
'          ExtractCode L_CodeLine, CommentStore
'          arrDimBlock(I) = L_CodeLine
'          arrDimComment(I) = CommentStore
'        End If
'      Next I
'      ArrSorts = Array("Boolean", "Byte", "Integer", "Long", "Double", "Single", "String", "Variant")
'      For I = 0 To UBound(ArrSorts)
'        For J = 0 To UBound(arrDimBlock)
'          If MultiRight(arrDimBlock(J), True, ArrSorts(I)) Then
'            'If lngTop <> J Then
'            '  If Not MultiRight(arrDimBlock(lngTop), True, ArrSorts(I)) Then
'                SwapVariants arrDimBlock(J), arrDimBlock(lngTop)
'                SwapVariants arrDimComment(J), arrDimComment(lngTop)
'                lngTop = lngTop + 1
'                Hit = True
'            '  Else
'            '    lngTop = lngTop + 1
...
'      If Hit Then
'        For J = 0 To UBound(arrDimBlock)
'          arrDimBlock(J) = arrDimBlock(J) & arrDimComment(J)
'        Next
'        J = 0
'        For I = BlockEnd - UBound(arrDimBlock) To BlockEnd
'          ArrProc(I) = arrDimBlock(J)
'          J = J + 1
'        Next
'      End If
'    End If
'      End If
'    DimSort = Join(CleanArray(ArrProc), vbNewLine)
'End Function
'Private Function DimControlDefaultProp(ByVal ModuleNumber As Long,
''                                       ByVal strA As String,
''                                       Comp As VBComponent) As String
'  Dim CommentStore     As String
'  Dim lngTmpIndex      As Long
'  Dim tmpB             As Variant
'  Dim I                As Long
...
'  Dim strComentId      As String
'  Dim StrFullCtrlID    As String
''This has to be here to stop DimUnused detecting the control names
''ver 2.0.2 thanks to necro static for bug which inserted extra spaces in string literals
'' and while hunting this bug (with the code necro provided) found a nasty one that missed
'' controls if they where the last word in the codeline
'  TmpA = Split(strA, vbNewLine)
'  If dofix(ModuleNumber, UpdateCtrlDefProp) Then
'' If IsControlName(strA) Then
'    ExistingDimArray = BuildExistingDimArray(strA)
'    MaxFactor = UBound(TmpA)
'    If MaxFactor > 0 Then
'      For I = 0 To MaxFactor
'' 1 means that it will not attack parameters with same name as controls(detected in parameteres)
'        WorkingMessage "Update Default Properties", I, MaxFactor
'        CommentUpdated = False
'        strComentId = ""
'        StrFullCtrlID = ""
'        UpDated = False
...
'            If isProcHead(L_CodeLine) Then
''fixed ver1.1.40 crashed if prochead used line continuation
'              ProcName = GetProcNameStr(L_CodeLine)
'              LParam = ParamArrays(GetWholeLineArray(TmpA, I, lngJunkMe), VariableOnlyParam)
'              GoTo NoSuspectCode  ' no suspect code; so get next line
'            End If
''CommentStore = CommentClip(L_Codeline)
...
''TmpB = ExpandForDetection2(L_Codeline)
'            tmpB = Split(ExpandForDetection(L_CodeLine))
'            If Not PotentialControlDefault(Comp, tmpB) Then
'              GoTo NoSuspectCode  ' no suspect code; so get next line
'            End If
'            CompactIndexes tmpB
'            TmpBSize = UBound(tmpB)
...
'                J = J + 1
''ver 1.1.25 because J may be reset within the For loop to excess value
'                If J <= TmpBSize + 1 Then
'                  If LenB(tmpB(J)) Then
'                    If Not IsPunct(CStr(tmpB(J))) Then
'                      If InStr(tmpB(J), Chr$(34)) = 0 Then
'                        If IsControlName(tmpB(J)) Then
'                          lngTmpIndex = CntrlDescMember(tmpB(J))
'                          lngTmpIndex = CntrlDescMember2(tmpB(J), Comp.Name)
''Deal with Properties of Forms/UserControls being used without reference to the Form/UserControl
''-------------------------------
'                          If lngTmpIndex = -1 Then
''Not a control test for FreeStandingProperty
'                            If IsControlProperty(tmpB(J)) Then
'                              If IsComponent_ControlHolder(Comp) Then
'                                FreeStandingProperty Comp, tmpB, J, L_CodeLine, CFMsg3, ExistingDimArray, TmpBSize
'                                GoTo DoMessage
'                              End If
'                            End If
'                           Else
...
'                              If InQSortArray(ExistingDimArray, tmpB(J)) Then
''ver 1.1.31 Control and Local Dim have same name
'                                TmpA(I) = L_CodeLine & SafeReattachComment(TmpA(I), CommentStore)
'                                TmpA(I) = SmartMarker(TmpA, I, WARNING_MSG & "The variable '" & tmpB(J) & "' has the same name as a Control." & vbNewLine &
''                                 "While legal this makes code harder to read and may prevent some Code Fixer actions." & SuggestNewName(tmpB(J), TmpA(DimLine)), MAfter)
'                                DimLine = FindDimLine(tmpB(J), TmpA)
'                                If DimLine > -1 Then
'                                  If InStr(TmpA(DimLine), WARNING_MSG & "The variable '" & tmpB(J) & "'") = 0 Then
''only comment once (while still in array the msg is part of the line
...
'                               ElseIf IsInArray(tmpB(J), LParam) Then
'                                TmpA(I) = L_CodeLine & SafeReattachComment(TmpA(I), CommentStore)
'                                TmpA(I) = SmartMarker(TmpA, I, WARNING_MSG & "Control and a Parameter '" & tmpB(J) & "' have the same name." & vbNewLine &
''                                 "While legal this makes code harder to read and may prevent some Code Fixer actions." & SuggestNewName(tmpB(J), TmpA(DimLine)), MAfter)
'                                Exit For
'                              End If
'                             ElseIf IsControlProperty(tmpB(J)) And Not IsVBControlRoutine(ProcName) Then
'                              TmpA(I) = L_CodeLine & SafeReattachComment(TmpA(I), CommentStore)
'                              TmpA(I) = SmartMarker(TmpA, I, WARNING_MSG & "The variable '" & tmpB(J) & "' has the same name as a Control Property." & vbNewLine &
''                               "While legal this makes code harder to read and may prevent some Code Fixer actions." & SuggestNewName(tmpB(J), TmpA(DimLine)), MAfter)
'                              DimLine = FindDimLine(tmpB(J), TmpA)
'                              If DimLine > -1 Then
'                                If InStr(TmpA(DimLine), WARNING_MSG & "The variable '" & tmpB(J) & "'") = 0 Then
...
'                             ElseIf isVBReservedWord(ProcName) Then
'                              TmpA(I) = L_CodeLine & SafeReattachComment(TmpA(I), CommentStore)
'                              TmpA(I) = SmartMarker(TmpA, I, WARNING_MSG & "The variable '" & tmpB(J) & "' has the same name as a VB Reserved WOrd." & vbNewLine &
''                               "While legal this makes code harder to read and may prevent some Code Fixer actions." & SuggestNewName(tmpB(J), TmpA(DimLine)), MAfter)
'                              DimLine = FindDimLine(tmpB(J), TmpA)
'                              If DimLine > -1 Then
'                                If InStr(TmpA(DimLine), WARNING_MSG & "The variable '" & tmpB(J) & "'") = 0 Then
...
''ver 1.1.42 tests for control names that are unsafe (for Code Fixer)
'                            With Comp
'                              If Not IsParameter2(tmpB, J, .Name) Then
'                                RefUpdated = ControlNeedsFormRef(tmpB(J), .Name)
'                                InsertDefProp L_CodeLine, tmpB(J), DefProp, UpDated, CommentUpdated, RefUpdated
'                                If CommentUpdated Then
'                                  strComentId = tmpB(J)
...
'                End If
'AlreadyHasOne:
'              Loop Until J >= TmpBSize
'            End If
''-------------------------
'DoMessage:
'            If LenB(CFMsg) Then
'              TmpA(I) = L_CodeLine & SafeReattachComment(TmpA(I), CommentStore)
'              TmpA(I) = SmartMarker(TmpA, I, CFMsg, MAfter)
'              CFMsg = vbNullString
'             ElseIf CommentUpdated Then
'              TmpA(I) = SmartMarker(TmpA, I, WARNING_MSG & "Problem identifying " & strInSQuotes(strComentId) & "." & vbNewLine &
''               RGSignature & IIf(LenB(StrFullCtrlID), "Variable has same name as Control " & StrFullCtrlID & ".", "Variable has same name as a Control/VB command") & IIf(LenB(StrFullCtrlID), vbNullString, vbNewLine &
''               RGSignature & "or a Control without a detectable default Property."), MAfter)
'              CommentUpdated = False
'              strComentId = ""
'              StrFullCtrlID = ""
'            End If
'            If LenB(CFMsg2) Then
'              TmpA(I) = SmartMarker(TmpA, I, CFMsg2, MAfter)
'              CFMsg2 = vbNullString
'            End If
'            If LenB(CFMsg3) Then
'              TmpA(I) = L_CodeLine & SafeReattachComment(TmpA(I), CommentStore)
'              If InStr(TmpA(I), ") .") Then
'                TmpA(I) = Safe_Replace(TmpA(I), ") .", ").")
'              End If
'              TmpA(I) = SmartMarker(TmpA, I, CFMsg3, MAfter)
'              CFMsg3 = vbNullString
'            End If
'          End If
'        End If
...
'' End If
'  End If
'  DimControlDefaultProp = Join(CleanArray(TmpA), vbNewLine)
'End Function

':)Code Fixer V2.9.6 (9/02/2005 2:59:54 AM) 5 + 2471 = 2476 Lines Thanks Ulli for inspiration and lots of code.
