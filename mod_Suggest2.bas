...
Public Function InternalEvidence(varTest As Variant) As String
  ...
  If InStr(varTest, "Join") And InStr(varTest, "Split") Then
   ...
   ElseIf isRefLibKnownVBConstant(varTest) Then
    ...
    End If
   ElseIf InstrArray(varTest, DQuote, "$", " & ", "Join", "InputBox", "vbNullString") Then
    InternalEvidence = " As String"
   ...
   ElseIf MultiLeft(varTest, True, ".ScaleHeight", ".ScaleWidth", ".Height", ".Width", ".ForeColor", ".BackColor", "+") Then
    InternalEvidence = " As Long"
   ElseIf InstrArray(varTest, "MsgBox", "vbOK", "vbYes", "vbNo", "vbCancel", "vbAbort", "vbRetry", "vbIgnore") Then
    InternalEvidence = " As VbMsgBoxResult"
   ElseIf InstrArray(varTest, "Sqr(", "Sqr", "Timer", "Round", "Shell") Then
    InternalEvidence = " As Double"
   ElseIf InstrArray(varTest, "/", "*") Then
...
Public Function SearchRoutineArray(ByVal VarRoutine As Variant, _
                                   ByVal varFind As Variant) As Boolean

  Dim I       As Long
  Dim arrLine As Variant

  arrLine = Split(VarRoutine, vbNewLine)
  For I = LBound(arrLine) To UBound(arrLine)
    If InstrAtPosition(arrLine(I), varFind, ipAny, True) Then
      SearchRoutineArray = True
      Exit For
    End If
  Next I
...
':)Code Fixer V2.9.6 (9/02/2005 3:05:30 AM) 18 + 525 = 543 Lines Thanks Ulli for inspiration and lots of code.
