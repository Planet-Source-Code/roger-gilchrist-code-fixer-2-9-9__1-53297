  ...
  If InstrAtPositionSetArray(Cline, ipLeftOr2ndOr3rd, True, ArrFuncPropSub) Then
    If ExtractCode(Cline) Then
      ...
      For I = LBound(arrLine) To UBound(arrLine)
        If ArrayMember(arrLine(I), "Function", "Property", "Sub") Then
              ...
              If UBound(arrLine) > 1 Then
                Select Case arrLine(1)
                 Case "="
                  If LenB(InternalEvidence(arrLine(2))) Then
                   ...
                   ElseIf UBound(arrLine) > 2 Then
                    ...
                    If IsDeclaration(CStr(arrLine(2)), "Public") Then
                     ...
                     ElseIf InstrArray(JoinPartial(arrLine, 3, UBound(arrLine)), "/", "*") Then
                      GetDimTypeFromInternalEvidence = " As Single"
                      Exit For
                     ElseIf InstrArrayWholeWord(JoinPartial(arrLine, 3, UBound(arrLine)), "Mod") Then
                  ...
                  End If
                 Case varTest
                  If InstrArray(JoinPartial(arrLine, 3, UBound(arrLine)), "/", "*") Then
                    GetDimTypeFromInternalEvidence = " As Single"
                    Exit For
                   ElseIf InstrArrayWholeWord(JoinPartial(arrLine, 3, UBound(arrLine)), "Mod") Then
                    GetDimTypeFromInternalEvidence = " As Long"
                   ...
                   ElseIf arrLine(0) = "For" Then 'NOT INSTRARRAY(JOINPARTIAL(ARRLINE,...
...
Public Function GetProcClassStr(ByVal strCode As String) As String
  ...
  If isProcHead(strCode) Then
    ...
    Do While ArrayMember(LeftWord(strCode), "Public", "Private", "Friend", "Static", "Set", "Let", "Get")
...
Public Function GetProcNameStr(ByVal strCode As String) As String
  ...
  Do Until ArrayMember(LeftWord(strTmp), "Public", "Private", "Friend", "Static", "Sub", "Function", "Property", "Set", "Let", "Get")
  ...
  If LenB(strCode) Then
    Do While ArrayMember(LeftWord(strTmp), "Public", "Private", "Friend", "Static", "Sub", "Function", "Property", "Set", "Let", "Get")
      strTmp = Trim$(Mid$(strTmp, Len(LeftWord(strTmp)) + 1))
      If LenB(strTmp) = 0 Then
        Exit Do
      End If
  ...
  If TPos > -1 Then
   ...
   Else
    If InStr(varOld, LBracket) > 1 Then
      'TPos = ArrayPos(Mid$(varOld, InStr(varOld, LBracket) - 1, 1), TypeSuffixArray)
...
':)Code Fixer V2.9.6 (9/02/2005 2:59:08 AM) 1 + 712 = 713 Lines Thanks Ulli for inspiration and lots of code.
