...
Private Sub DecIndent()
  ...
  For I = TopOfRoutine + 1 To UBound(ArrProc)
    ...
    If MultiLeft(ArrProc(I), True, WARNING_MSG & "Incomplete ") Then
      ArrProc(I) = ""
    End If
  Next I
...
Private Sub DoMatchTrigger(strData As String, _
  ...
  If MultiRight(strData, True, strTest) Then
    bResult = True
    RLen = Len(strTest)
  ...
  Select Case strTrigger
   ...
   Case "End"
    ...
    If UBound(ErrStruct) > -1 Then
      If MultiLeft(strCode, True, "End " & ErrStruct(UBound(ErrStruct))) Then
        MatchTrigger = True
...
':)Code Fixer V2.9.6 (9/02/2005 3:11:51 AM) 17 + 605 = 622 Lines Thanks Ulli for inspiration and lots of code.
