                ...
                If LModule.Find(Targets(K), StartLine, 1, -1, -1, bWholeWordonly, bCaseSensitive, False) Then
                  Do
                    ...
                    If Left$(LModule.Lines(StartLine, 1), 8) = "End With" Then
                      Exit Do
                    End If
                  Loop While GetWholeCaseMatchCodeLine(Proj.Name, Comp.Name, strCtrl, L_CodeLine, StartLine)
                End If
              Next K
            End If
          End If
...
Private Sub ChangeControlOnModules(ByVal strCtrl As String, _
                                   ...
                                   Optional ByVal RenameArray As Boolean = False)
              ...
              If LModule.Find(strCtrl, StartLine, 1, -1, -1, bWholeWordonly, bCaseSensitive, False) Then
                ...
                Do
                  ...
                  If LModule.Find(strCtrl, StartLine, 1, -1, -1, bWholeWordonly, bCaseSensitive, False) Then
                    ...
                    If Not JustACommentOrBlank(L_CodeLine) Then
                      If Not ProhibitedControlUpdate(L_CodeLine, strCtrl) Then
                        If InStr(L_CodeLine, strCtrl) <> InStr(L_CodeLine, strNewName) Then
    ...
    If .lstPrefixSuggest.ListIndex < 0 Then
     ...
     Else
      ...
      If .lsvAllControls.SelectedItem.Index <> .lsvAllControls.ListItems.Count Then
        If Not MultiLeft(.txtCtrlNewName.Text, True, CurName) Then
      ...
      For Each ctrla In ArrayCntrls
        ...
        If Targetcontrol.Properties("index").Value = 1 Then
          ...
          If UBound(ArrayCntrls) = 0 Then
            ...
            FixNewlyIndexed strNewName, strHomeComp, True
                ...
                If DoControlRename Then
                  If .Properties("index").Value > -1 Then
                    If lngNewIndex < 1 Then
                      FixNewlyIndexed strNewName & strInBrackets(.Properties("index").Value), strHomeComp
                    End If
                  End If
                End If
              End If
           ...
           Else
            If UBound(ArrayCntrls) > 0 Then
             ...
             ElseIf UBound(ArrayCntrls) = 0 Then
              ...
              End If
              If Targetcontrol.Properties("index").Value > -1 Then
                FixNewlyIndexed strNewName & strInBrackets(Targetcontrol.Properties("index").Value), strHomeComp
                DoControlRename = True
                SetControlData LngCurrentControl, Targetcontrol, GetComponent(CntrlDesc(LngCurrentControl).CDProj, CntrlDesc(LngCurrentControl).CDForm), vbf
              End If
            End If
            ...
            If LModule.Find(strError, StartLine, 1, -1, -1, bWholeWordonly, bCaseSensitive, False) Then
              ...
              If ExtractCode(L_CodeLine, strComment) Then
                ...
                If CntrlDesc(LngCurrentControl).CDForm = Comp.Name Then
                  ...
                  If ArrayMember(Right$(L_CodeLine, 2), "()", ContMark) Or isProcHead(L_CodeLine) Then
                    LUpdated = True
                  End If
                  If InsertEmptyCaseStructure Then
                    If Right$(L_CodeLine, 2) = "()" Then
                     ...
                     Else
                      If InStr(L_CodeLine, strCorrect & "_" & strEvent) Then
                        If MultiLeft(L_CodeLine, True, "Call " & strCorrect) Then
                    ...
                    If isProcHead(L_CodeLine) Then
                      ...
                      Do Until MultiLeft(LModule.Lines(lngLegalEnd, 1), True, strLegalEnd)
                   ...
                   Else
                    If LUpdated Then
                      ...
                      End If
                      GetorDeleteProcedureCode StartLine, GetCodeLine, LModule, L_CodeLine, strError, strEvent, StrOldParam, strHomeComp
                      LUpdated = False
                    End If
                  End If
                End If
...
Private Sub GetorDeleteProcedureCode(ByVal StartLine As Long, _
  ...
  strLegalEnd = GetLegalEnd(L_CodeLine)
  StrProdCode = "Case " & ExtractNumber(strError)
  Do Until MultiLeft(LModule.Lines(lngLegalEnd, 1), True, strLegalEnd)
    StrProdCode = StrProdCode & vbNewLine & LModule.Lines(lngLegalEnd, 1)
    lngLegalEnd = lngLegalEnd + 1
  Loop
  ...
  If Not ArrayMember(StrScope, "Public", "Private", "Static", "Friend") Then
    StrScope = ""
  End If
  FixNewRoutine WordInString(strLegalEnd, 2), Left$(strError, InStr(strError, LBracket) - 1) & strEvent, StrProdCode, RGSignature & "INSERT CODE HERE", StrOldParam, StrScope, strHomeComp
...
Public Function HasNoHungarianPrefix(ByVal lngIndex As Long, _
  ...
  If Len(strCorrectPrefix) Then
    HasNoHungarianPrefix = Not MultiLeft(LCase$(CntrlDesc(lngIndex).CDName), False, strCorrectPrefix)
   Else
...
Public Function IsWholeWord(ByVal strCode As String, _
  ...
  If TPos = 1 Then
   ...
   ElseIf IsPunct(Mid$(strCode, TPos - 1, 1)) Then
    If MultiRight(strCode, True, strF) And TPos = Len(strCode) - Len(strF) + 1 Then
      IsWholeWord = True
...
Public Function NoHungarianPrefix(ByVal strObj As String) As String
  ...
  If Len(HPref) Then
    If strObj <> CntrlDesc(LngCurrentControl).CDClass Then
      If MultiLeft(strObj, False, HPref) Then
...
Private Function PreviousWith(LMod As CodeModule, _
  ...
  For I = StartLine To TopOfProc Step -1
    If MultiLeft(Trim$(LMod.Lines(I, 1)), True, "With ") Then
      strTest = Trim$(LMod.Lines(I, 1))
      For Each TmpMemb In Targets
        If MultiLeft(strTest, True, TmpMemb) Then
          PreviousWith = True
          Exit For 'unction
        End If
      Next TmpMemb
...
Private Function ProhibitedControlUpdate(strCode As String, _
                                         strCtrl As String) As Boolean
  ...
  If ArrayMember(strCtrl, "Exit", "Line") Then
    Select Case strCtrl
     Case "Exit"
      ProhibitedControlUpdate = ArrayMember(WordInString(strCode, 2), "Sub", "Function", "Property")
     Case "Line"
      ProhibitedControlUpdate = ArrayMember(WordInString(strCode, 2), "Input")
    End Select
  End If
...
Public Sub RenameControlsStandard(strOldName As String, _
                                  ...
                                  Optional RenameArray As Boolean = False)

  ChangeControlInWith strOldName, strNewName
  ChangeControlOnModules strOldName, strNewName, RenameArray
...
':)Code Fixer V2.9.6 (9/02/2005 2:56:57 AM) 27 + 1344 = 1371 Lines Thanks Ulli for inspiration and lots of code.
